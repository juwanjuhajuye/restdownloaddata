package com.example.jjbfather.jjjqsrt2go2gorestdownload.tablesale;
...
import com.example.jjbfather.jjjqsrt2go2gorestdownload.CloverMakingViewInPrinting;
import com.example.jjbfather.jjjqsrt2go2gorestdownload.GlobalMemberValues;
import com.example.jjbfather.jjjqsrt2go2gorestdownload.JJJ_SignPad;
import com.example.jjbfather.jjjqsrt2go2gorestdownload.LogsSave;
import com.example.jjbfather.jjjqsrt2go2gorestdownload.MainActivity;
import com.example.jjbfather.jjjqsrt2go2gorestdownload.MainMiddleService;
import com.example.jjbfather.jjjqsrt2go2gorestdownload.MssqlDatabase;
import com.example.jjbfather.jjjqsrt2go2gorestdownload.PaymentCreditCard;
import com.example.jjbfather.jjjqsrt2go2gorestdownload.R;
import com.example.jjbfather.jjjqsrt2go2gorestdownload.VoidCardList;
...
//                " select sum(sTotalAmount) from temp_salecart where holdcode = '" + str_holdCode + "' "));
        //09302024 use helper method
        d_TotalAmount = getTotalTransactionAmount();
...
//                " select sum(sTotalAmount) from temp_salecart where holdcode = '" + str_holdCode + "' "));
        //09302024 use helper method
        double totalBillAmount = getCurrentBillAmount();
        d_TotalAmount = getTotalTransactionAmount();
    ...
    //09302024 add some helper methods for getting current Bill Amount and total transaction Amount
    private double getCurrentBillAmount() {
        double totalBillAmount = GlobalMemberValues.getDoubleAtString(
                MssqlDatabase.getResultSetValueToString(
                        " select sum(billamount) from bill_list where holdcode = '" + str_holdCode + "' "
                )
    ...
    private double getTotalTransactionAmount() {
        double totalTransactionAmount = GlobalMemberValues.getDoubleAtString(MssqlDatabase.getResultSetValueToString(
                " select sum(sTotalAmount) from temp_salecart where holdcode = '" + str_holdCode + "' "));
                ...
                case R.id.tablesale_splitmerge_pay_by_bills : {
                    ...
                    if (mSelectedBillBoxValue.size() == 1) {
                        ...
                        if (str_Billsplit_Type == "0" || str_Billsplit_Type.equals("0")) {
                            ...
                            //09302024 not used with the new checking method
...
//                            double tempSplitCntByMenu = GlobalMemberValues.getDoubleAtString(tablesale_split_evenly_txt2.getText().toString());
                            ...
                            //093024 check whether split is complete by transaction amount the bills add upto,
                            //Not by how many bills are there
                            Double currBillAmount = getCurrentBillAmount();
                            Double totalTransactionAmount = getTotalTransactionAmount();
                            //if (billlistcnt < tempSplitCntByMenu) {
                            if (currBillAmount < totalTransactionAmount){
