...
import com.example.jjbfather.jjjqsrt2go2gorestdownload.star.StarPrintStart;
    ...
    public static void backupDatabase(boolean paramOpenDialog) {
        ...
        try {
            ...
            if (sd.canWrite()) {
                ...
                //06032024 only if the device is using wal mode, back up -wal and -shm files,
                ...
                //05312024 Loop transferFrom method to make sure all the content transfers
                for(long count = currentDB.length(); count > 0L;){
                    final long transferred = dst.transferFrom(
                            src, dst.position(), count);
                    dst.position(dst.position() + transferred);
